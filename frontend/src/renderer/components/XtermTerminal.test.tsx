...
const state = vi.hoisted(() => ({
	...
	lastTerminal: null as null | {
		...
		buffer: {
			active: { baseY: number; type: string; viewportY: number };
...
vi.mock("@xterm/xterm", () => ({
	Terminal: class FakeTerminal {
		...
		buffer = { active: { baseY: 0, type: "normal", viewportY: 0 }, normal: { length: 0 } };
		scrollLines = vi.fn();
...
describe("XtermTerminal", () => {
	...
	it("does not render the app-owned terminal scrollbar outside macOS", () => {
		const { container } = render(<XtermTerminal theme="dark" />);

		expect(container.querySelector(".terminal-scrollbar")).toBeNull();
	});

	it("renders a slim draggable scrollbar on macOS", () => {
